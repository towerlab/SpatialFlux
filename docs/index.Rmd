...
## Prerequisites
SpatialFlux package requires a spatial dataset already processed and integrated. For help in generating a Seurat object, please visit [Seurat](https://satijalab.org/seurat/articles/spatial_vignette) vignette.

## Introduction 
For this vignette tutorial, we provide a ST example dataset of mice subjected to tibia osteotomy obtained from [Rios et al., 2024](https://insight.jci.org/articles/view/176802#BIBL). Additional files to run the vignette can be found at zenodo repository [here](https://doi.org/10.5281/zenodo.14983368).

# Analysis Approaches
SpatialFlux supports different strategies for delimiting spatial reference spots by: 1) manually defining references spots using the packages built-in Shiny App; 2) selecting single cell types based on cluster identities, or 3) manually defining references lines using imaging *softwares* such as Fiji/ImageJ. For tutorial purposes, we will first show how to select spots using the three different approaches possible. Then, we'll present how to investigate and visualize gene expression and module score across these established distance gradients.

## 1. Individual Spots Selection approach
...
![](../images/plots/shinyapp_noselection.png)
...
![](../images/plots/shinyapp_selection.png)
...
# Calculate SpatialFlux between cluster and reference
sp <- SpatialShinyFlux(target, ref)
sp
```
...
gradient_vis +
  theme(aspect.ratio = ratio)
```
![](../images/plots/selection_gradient.png)

```sh
# Calculate gradient distance values and add SpatialFlux (sf) values into metadata
...
gradient_calc <- subset(gradient_calc, subset = sf != 0)

# Continue to in "Gene Expression Assessment & Visualization" section below
```

## 2. Cluster selection approach
...
2.2  Import coordinates to be used as references, as well as clusters of interest to calculate distance in, and run ```Spatial2Flux``` for gradient distance calculation:
```sh
...
# Calculate SpatialFlux between clusters of interest ----
sp  <- Spatial2Flux(target, ref)
sp
...
gradient_vis  +
 theme(aspect.ratio = ratio)
```
![](../images/plots/cluster_gradient.png)

```sh
# Add sf values into metadata
gradient_calc <- SpatialVis(demo, sp, spatial.by = "rel", slice = "slice1", return_obj = T)
View(gradient_calc@meta.data)

# Subset out spots not selected as target tissue where spatialflux (sf) value were not calculated
gradient_calc <- subset(gradient_calc, subset = sf != 0)

# Continue to "Gene Expression Assessment & Visualization" section below
```

## 3. Fiji/ImageJ approach
...
```
3.3 At this step, SpatialFlux distance are ready to be calculated. First, import your cluster type coordinates as reference and calculate the spots distances against other cluster. 
```sh
# Calculate minimum spots distances from reference line of interest
file <- read.csv("coord_out/Woven_slice1_coordinates.csv")
sp <- SpatialFlux(data, reference = "Cartilage", compare = file)
sp
```

3.4 SpatialFlux distance gradient can be assessed using:
```sh
ratio <- ScaleRatio(demo)

gradient_vis <- SpatialVis(demo, sp, spatial.by = "rel", slice = "slice1", return_obj = F)
gradient_vis + theme(aspect.ratio = ratio)
```
![](../images/plots/gradient_lineref.png)
3.5 Adding spatialflux values into metadata and subsetting only cells with sf values:
```sh
# Visualization spatialflux gradient ----
gradient_calc <- SpatialVis(demo, sp, spatial.by = "rel", slice = "slice1", return_obj = T)
gradient_calc <- subset(gradient_calc, subset = sf != 0)
```
# Gene Expression Assessement & Visualization
In this step, independently of spots selection approach use, sf values should be included in ```demo@meta.data```. Consequently, pseudotime can be calculated and gene expression analysis visualized as heatmap across distance gradient. For instance: 
```sh
# Calculate pseudotime and visualize genes differentialy expressed across distance gradient
ps <- PseudoM3Flux(gradient_calc, assay = "SCT", values = "sf", return_obj = F)
hm <- gradient_heatmap(ps, rownames = F)
```
![](../images/plots/heatmap_oficial.png)
...
ps <- PseudoM3Flux(gradient_calc, assay = "SCT", values = "sf", return_obj = T)
GeneVis(file = ps, column = c("Acan", "Col2a1", "Bglap"), span = 0.5, signal = "gene")
```

![](../images/plots/genes_curve_plot.png)
```sh
# Gene list pathways
...
```
![](../images/plots/pathway_curve_plot.png )


# Spatial Transcriptomics Visium HD
SpatialFlux package also supports 10x genomics Visium HD analysis allowing user to calculate distance gradients in a true single cell resolution. The SpatialFlux rationale for HD assays remains with minor modifications from standard cluster-based or image-based approaches. For instance:
...
hd <- readRDS("path/seurat_object.rds")
hd <- UpdateSeuratObject(hd)

subset2LabelsHD(hd, cluster = c("CellType1", "CellType2"))

ref <- CoordMergeHD("coord_out", pattern = "CellType1")
other <- CoordMergeHD("coord_out", pattern = "CellType2")

sp <- Spatial2FluxHD(other, ref)

gradient_vis <- SpatialVis(hd, sp, spatial.by = "rel", slice = "slice1", return_obj = F) 
gradient_vis

gradient_calc <- SpatialVis(demo, sp, spatial.by = "rel", slice = "slice1", return_obj = T)
gradient_calc <- subset(gradient_calc, subset = sf != 0)

# Continue as in "Gene Expression & Visualization"
```
...
sp <- SpatialFluxHD(data, reference = "CellType1", compare = file)
sp
...
## Attention
Despite support for 10x genomics Visium HD protocol, SpatialFlux calculation might take considerable computing time and resources due to true single cell resolution datasets size and complexity. Below, we provide a brief benchmark of computing time as number of cells increases. Overall, we highly recommend having at disposal at least 6 CPU cores and 64gb RAM.
![](../images/plots/benchmark_cellsn.jpeg)

## Monocle Usage
SpatialFlux package was originally conceived using ```monocle``` for spatialvalues calculation mainly to its reliability and heatmap plotting facility. However, monocle's newer versions such as ```monocle3``` brought series of performance and analysis enhancements. Thus, we additionally provide a monocle wrapper function that allows equivalent spatialflux calculation in the standard SpatialFlux pipeline. For instance:

```sh
library(monocle)

ps <- Pseudo2Flux(gradient_calc, assay = "SCT", return_obj = F)
p <- plot_pseudotime_heatmap(ps, num_clusters = 2, cores = 2, show_rownames = F, return_heatmap = T)
p
```
![](../images/plots/monocle.png)
...
```sh
ps <- Pseudo2Flux(fracture, assay = "SCT", return_obj = T)
```
...
[1] SpatialFlux_0.0.0.9000 testthat_3.2.0         devtools_2.4.5         usethis_2.2.2         
