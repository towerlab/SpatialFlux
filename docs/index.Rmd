...
```sh
# Calculate gradient distance values and add SpatialFlux (st) values into metadata
...
# Subset out spots not selected as target tissue where spatialflux (st) value were not calculated
gradient_calc <- subset(gradient_calc, subset = st != 0)
...
3.5 Adding spatialflux values into metadata and subsetting only cells with st values:
```sh
...
In this step, independently of spots selection approach use, st values should be included in ```demo@meta.data```. Consequently, pseudotime can be calculated and gene expression analysis visualized as heatmap across distance gradient. For instance: 
```sh
# Calculate pseudotime and visualize genes differentialy expressed across distance gradient
ps <- PseudoM3Flux(gradient_calc, assay = "SCT", values = "st", return_obj = F)
...
ps <- PseudoM3Flux(gradient_calc, assay = "SCT", values = "st", return_obj = T)
